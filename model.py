...
from Loss import position_loss
...
def STM_model(weights='pascal_voc', input_tensor=None, input_shape=(512, 512, 3), classes=21, backbone='mobilenetv2',
...
train_dataset = tf.data.TFRecordDataset('E:\\fift_paper_dataset\\train_dataset\\train_dataset.tfrecords')    # read TFRecord file
val_dataset=tf.data.TFRecordDataset('E:\\fift_paper_dataset\\train_dataset\\val_dataset.tfrecords')    # read TFRecord file


feature_description = { 'train_image': tf.io.FixedLenFeature([], tf.string),
                        'train_label': tf.io.FixedLenFeature([], tf.string)
                        }

def train_image_parse_image_function(example_proto):
     # Parse the input tf.Example proto using the dictionary above.
     parse=tf.io.parse_single_example(example_proto,feature_description)
     fixed = tf.io.decode_raw(parse["train_image"], tf.float32)
     fixed = tf.reshape(fixed, [256, 256, 2])
     return fixed


def train_label_parse_image_function(example_proto):
    # Parse the input tf.Example proto using the dictionary above.
    parse = tf.io.parse_single_example(example_proto, feature_description)
    moving = tf.io.decode_raw(parse["train_label"], tf.float32)
    moving = tf.reshape(moving, [256, 256, 1])
    return moving


train_iamge_parsed_image_dataset = train_dataset.map(train_image_parse_image_function)
train_label_parsed_image_dataset = train_dataset.map(train_label_parse_image_function)

train_ds=tf.data.Dataset.zip((train_iamge_parsed_image_dataset,train_label_parsed_image_dataset)).shuffle(100).batch(1)
train_ds=train_ds.prefetch(tf.data.experimental.AUTOTUNE)

val_image_parsed_image_dataset = val_dataset.map(train_image_parse_image_function)
val_label_parsed_image_dataset =val_dataset.map(train_label_parse_image_function)

val_ds=tf.data.Dataset.zip((val_image_parsed_image_dataset,val_label_parsed_image_dataset)).shuffle(100).batch(1)
val_ds=val_ds.prefetch(tf.data.experimental.AUTOTUNE)


model = STM_model(input_shape=(256, 256, 2), classes=1, backbone='xception',activation="sigmoid")
loss=position_loss()
model.compile(optimizer=tf.keras.optimizers.Adam(learning_rate=0.0001),loss=loss,metrics=["accuracy"])

checkpoint_path="weight_model\\model2.{epoch:02d}.hdf5"
checkpoint=tf.keras.callbacks.ModelCheckpoint(filepath=checkpoint_path,verbose=1,save_weights_only=False,save_freq=5)
model.fit(train_ds,validation_data=val_ds,validation_steps=3,epochs=100,verbose = 1,callbacks=[checkpoint])
