...
with tf.io.TFRecordWriter(tfrecord_file) as writer:
    for i in range(image.shape[0]-400, image.shape[0]):
        ...
        for i in range(25):
            ...
            # cv2.imwrite("E:\\fift_paper_dataset\\train_dataset\\display\\" + str(count) + "_optical_.tif", optical_image)
            # cv2.imwrite("E:\\fift_paper_dataset\\train_dataset\\display\\" + str(count) + "_sar_.tif", sub_sub_sar / 255)
            # cv2.imwrite("E:\\fift_paper_dataset\\train_dataset\\display\\" + str(count) + "_label_.tif", label_iamge)
