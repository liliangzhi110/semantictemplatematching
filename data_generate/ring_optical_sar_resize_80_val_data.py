...
with tf.io.TFRecordWriter(tfrecord_file) as writer:
    for i in range(image.shape[0] - 400,image.shape[0]):
        ...
        affine_transform_point2 = np.float32(
            [[circle_centrio_y - 60 + np.random.randint(-4, 4), circle_centrio_x + np.random.randint(-4, 4)],
        ...
        con_image = np.concatenate((tran_optical_image.reshape((256, 256, 1)), sub_sub_sar.reshape((256, 256, 1))),
                                   axis=2) / 255
