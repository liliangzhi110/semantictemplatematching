...
with tf.io.TFRecordWriter(tfrecord_file) as writer:
    for i in range(image.shape[0] - 400, image.shape[0]):
        ...
        for i in range(20):
            circle_centrio_x = np.random.randint(0, 95)
            circle_centrio_y = np.random.randint(0, 95)
            ...
            con_image = np.concatenate((optical_image.reshape((256, 256, 1)), sub_sub_sar.reshape((256, 256, 1))),
                                       axis=2) / 255
